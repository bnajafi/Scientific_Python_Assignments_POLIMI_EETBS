...
results_heating_Piacenza = np.array([QtotOpaque_winter, Qfen_heating_load, Output_Inf_Vent["Results"]["Sensible Infiltration/Ventilation Heating Load [W]"],
                                    Losses["Results"]["Heating distribution losses"]])
labels_heat = ["opaque", "windows", "infiltration/ventilation", "distributed losses"]

results_cooling_Piacenza = np.array([QtotOpaque_summer, Qfen_cooling_load, Output_Inf_Vent["Results"]["Internal Gain, sensible [W]"], 
                                     Output_Inf_Vent["Results"]["Sensible Infiltration/Ventilation Cooling Load [W]"], 
                                     Losses["Results"]["Cooling distribution losses"], 
                                     Output_Inf_Vent["Results"]["Sensible Infiltration/Ventilation Cooling Load [W]"]])
labels_cool = ["opaque", "windows", "internal gains", "infiltration/ventilation", "distributed losses", "latent"]

plt.figure()
plt.pie(results_heating_Piacenza,explode=(0.01,0.01,0.01,0.01), labels = labels_heat, autopct = '%1.1f%%')
plt.title("HEATING LOADS PIACENZA")
plt.show()

plt.figure()
plt.pie(results_cooling_Piacenza,explode=(0.01,0.01,0.01,0.01,0.01,0.01), labels = labels_cool, autopct = '%1.1f%%')
...
inputs_DF = pd.read_csv("input_weather_Piacenza.csv",sep=";",index_col=0)
inputs_list = func.weather_data_calculator(inputs_DF)   #calculator can be seen in fenestration function script
...
windows_DF.to_csv("results_fenestration.csv",sep =";") #saving results...updating excel file
...
results_DF.to_csv("results_wholeRFL.csv",sep =";")
results_DF.to_html("results_wholeRFL.html")
